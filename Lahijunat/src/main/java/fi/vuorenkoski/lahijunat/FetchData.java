...
public class FetchData {
    ...
    public static ArrayList<DepartingTrain> DepartingTrainsFromStation(int UICCode) throws MalformedURLException, IOException, ParseException {
        ...
        URL url=new URL(baseAddress+"/live-trains/station/"+URLEncoder.encode(Station.StationShortName(UICCode),"UTF-8")
                +"?minutes_before_departure=120&minutes_after_departure=0&minutes_before_arrival=120&minutes_after_arrival=0&train_categories=Commuter");
        ...
        for (int i=0;i<data.length();i++) {
            ...
            for (int j=0;j<timeTableRows.length();j++) {
                if (timeTableRows.getJSONObject(j).getInt("stationUICCode")==UICCode
                    ...
                    scheduledTime=dateFormat.parse(timeTableRows.getJSONObject(j).getString("scheduledTime"));
                    if (timeTableRows.getJSONObject(j).has("liveEstimateTime")) {
                        liveEstimateTime=dateFormat.parse(timeTableRows.getJSONObject(j).getString("liveEstimateTime"));
            ...
            if (track.equals("")==false && scheduledTime.compareTo(currentTime)>0) 
                trainArray.add(new DepartingTrain(commuterLineId, trainNumer, destination, scheduledTime, liveEstimateTime, track, causes, cancelled));
        }
        // Sortataan junat lähtöajan mukaan
        return trainArray.stream().sorted().collect(Collectors.toCollection(ArrayList::new));
    }
}
